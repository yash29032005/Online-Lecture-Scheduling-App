...
const Instructors = () => {
  ...
  const filteredInstructors = instructors.filter((instruc) =>
    instruc.name?.toLowerCase().includes(search.toLowerCase())
  );

  const paginated = filteredInstructors.slice(
    (page - 1) * perPage,
    page * perPage
  );
  ...
  const handleDelete = async (id) => {
    try {
      const result = await axiosClient.delete(
        `${import.meta.env.VITE_API_URL}/api/instructor/delete/${id}`
      );
            ...
            <tbody>
              {paginated.map((instructor, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-2">{instructor.createdAt}</td>
